...
>*Answer 1:*
A visual inference can be made that a linear function would not fit the data well. Taking the average value would yield a saturating line with non constant slope. Specifically, the slope appears to decrease with time.
<br>
...
>*Answer 2:* 
The full model had lower estimates for the growth rate (0.989154 vs 1.17687 ) and NDVI at time zero (0.207522 vs 0.25107). In contrast, the full model a higher estimate for the gamma parameter (0.364746 vs 0.32371). The full model also had lower standard errors for the common parameters, all variables were significant for both models (p<0.001)
<br>
...
>*Answer 3:*
According to the ANOVA tests, the full model is the better model (negexpS) by virtue of the small p-value assosciated for the null hypothesis that the full model does not significantly reduce the RSS.
<br>

Q4: How many degrees of freedom are there in this ANOVA and why (i.e. what are they)?

>*Answer 4:*
The SSR aspect is tested with two degrees of freedom, corresponding to the two variables (p) introduced in the full model (A & $\phi$).
There are also degrees of freedom assosciated with the residuals, with df = (n-p-1).
<br>
...
>*Answer 5:*
The full model is overwhelmingly preferred by the AIC (smaller), suggesting that the additional parameters introduced contribute to the predictive power far more than they are detrimental to the model through increasing complexity (and thereby increasing risk of overfitting).
<br>
...
>*Answer 6:*
As specified in the help documentation, there is a requirement for log-likelihood to have been maximized. For the MLE models this holds, but the NLS models used a different means of fitting. Furthermore, the help documentation does specify that AIC values can be calculated and compared within the latter models, but are not comparable to our MLE models as a result of this violated assumption.
<br>